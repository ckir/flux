---- MODULE Fenced ----
\* PROTOTYPE (2026-09-16), not the V16 protocol: the fenced-publication variant, to be measured against
\* LockProtocol.tla before any spec change. Three differences, all of them consequences of one idea - the
\* lock is advisory and the last atomic operation is what decides the outcome:
\*   1. ownership is the file a process holds open, tested by identity against the lock path, not the record
\*      it reads back and not the OS-native lock;
\*   2. a takeover creates a NEW file and renames it over the lock, so a prior owner's in-flight calls land
\*      on a file no name reaches;
\*   3. publication writes a private file and renames it over the destination in one step, and the operation
\*      then re-checks its ownership before reporting success.
\* The destination is modelled here, which LockProtocol.tla does not do, so the properties can say what the
\* protocol is for: the destination always holds one operation's complete output, and an operation that
\* verified its ownership after publishing was not overwritten.
...
CONSTANTS
    ...
    TargetName,    \* the destination itself, which this prototype models
...
\* The private file an operation writes its output into before renaming it over the destination.
PubName(p) == <<"pub", p>>
Names == {LockName, DirLockName, TargetName} \cup {BrokenOf(p) : p \in Movers}
         \cup {TakeoverName(p) : p \in Procs} \cup {PubName(p) : p \in Procs}
Dirs == {P}
                   ...
                   /\ IF crashed[self]
                         ...
                         ELSE /\ IF StillOwned(self)
                                    THEN /\ checked' = [checked EXCEPT ![self] = TRUE]
                                         ...
                                         /\ pc' = [pc EXCEPT ![self] = "S99_pub_create"]
...
S99_pub_create(self) == /\ pc[self] = "S99_pub_create"
                        /\ IF crashed[self]
                              THEN /\ pc' = [pc EXCEPT ![self] = "publish_crashed"]
                                   /\ UNCHANGED << fs, refused >>
                              ELSE /\ LET r == FsCreate(fs, P, PubName(self), self, TRUE) IN
                                        IF r.ok
                                           THEN /\ fs' = r.fs
                                                /\ pc' = [pc EXCEPT ![self] = "S99_pub_write_begin"]
                                                /\ UNCHANGED refused
                                           ELSE /\ refused' = [refused EXCEPT ![self] = "RESTART"]
                                                /\ pc' = [pc EXCEPT ![self] = "S99_refuse_close"]
                                                /\ fs' = fs
...
S99_pub_write_begin(self) == /\ pc[self] = "S99_pub_write_begin"
                             /\ IF crashed[self]
                                   THEN /\ pc' = [pc EXCEPT ![self] = "publish_crashed"]
                                        /\ fs' = fs
                                   ELSE /\ fs' = FsWriteBegin(fs, At(fs, P, PubName(self))).fs
                                        /\ pc' = [pc EXCEPT ![self] = "S99_pub_write_end"]
                             /\ UNCHANGED << foreignObj, classified, ownerLive, 
                                             sawLive, seenRec, crashed, live, 
                                             holding, checked, writing, 
                                             pendingUnlink, checkStale, 
...
S99_pub_write_end(self) == /\ pc[self] = "S99_pub_write_end"
                           /\ IF crashed[self]
                                 THEN /\ pc' = [pc EXCEPT ![self] = "publish_crashed"]
                                      /\ fs' = fs
                                 ELSE /\ fs' = FsWriteEnd(fs, At(fs, P, PubName(self)), OwnRecord(self)).fs
                                      /\ pc' = [pc EXCEPT ![self] = "S99_pub_rename"]
                           /\ UNCHANGED << foreignObj, classified, ownerLive, 
                                           sawLive, seenRec, crashed, live, 
                                           holding, checked, writing, 
                                           pendingUnlink, checkStale, 
...
S99_pub_rename(self) == /\ pc[self] = "S99_pub_rename"
                        /\ IF crashed[self]
                              THEN /\ pc' = [pc EXCEPT ![self] = "publish_crashed"]
                                   /\ UNCHANGED << fs, published, refused >>
                              ELSE /\ LET r == FsRenameReplace(fs, P, PubName(self), TargetName) IN
                                        IF r.ok
                                           THEN /\ fs' = r.fs
                                                /\ published' = [published EXCEPT ![self] = TRUE]
                                                /\ pc' = [pc EXCEPT ![self] = "S99_verify"]
                                                /\ UNCHANGED refused
                                           ELSE /\ refused' = [refused EXCEPT ![self] = "RESTART"]
                                                /\ pc' = [pc EXCEPT ![self] = "S99_refuse_close"]
                                                /\ UNCHANGED << fs, published >>
                        /\ UNCHANGED << foreignObj, classified, ownerLive, 
                                        sawLive, seenRec, crashed, live, 
                                        holding, checked, writing, 
                                        pendingUnlink, checkStale, writeStale, 
...
Publish(self) == S99_check(self) \/ S99_pub_create(self)
                    \/ S99_pub_write_begin(self) \/ S99_pub_write_end(self)
                    \/ S99_pub_rename(self) \/ S99_verify(self)
                    \/ S99_write(self) \/ S240_5_inflight_lands(self)
...
\* ------------------------------------------------------------------------------------------
\* What the protocol is FOR (prototype, 2026-09-16). LockProtocol.tla cannot state these: it does not
\* model the destination, so it defends SingleWriter, a proxy, and collects an accepted window every
\* time the proxy breaks for a reason the spec tolerates.

\* The destination always holds ONE operation's complete output. Never a mixture of two, never a file
\* caught between the halves of a write, never an empty file: publication is a single rename of a
