...
| `SEED_RECOVER_UNCERTAIN` | a lock judged uncertain (a torn or empty record, or an owner the oracle cannot judge) is moved aside through 240.3, where 240.4 preserves it | `recovery`, POSIX, Owner and 2 Recoverers | `PlainNeverOwnsUncertain` |
...
The three `recovery` rows came from the test audit of 2026-09-14, which found that no run of the scenario could tell
its safety invariants from `TRUE`: an invariant no seeded run violates passes just as well with its ghost deleted. So
every safety invariant a scenario checks either has a seeded run in that scenario or states which scenario's seeded run
guards it. `SingleWriter`'s seeds need a takeover or a second recoverer judging by identity alone, so they belong to
...
|---|---|---|---|
| `recovery` | Owner (crashes), 2 Recoverers, PlainRun, Cleanup - **paired across four `check` runs, never all at once** (below) | POSIX, Windows | `NeverTornRead`, `NeverChecked`, `NeverRecoveredAfterCrash`, `NeverDeadLockWithPendingMover` |
